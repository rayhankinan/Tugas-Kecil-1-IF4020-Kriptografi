...
from ..utils import AllStringType, AllByteType, apply_dynamic_extvig_to_file, binary_to_num, num_to_binary, char_to_num
...
async def encrypt_file_service(key: AllStringType, file: UploadFile):
    async def encrypt_bytes(binary: AllByteType, counter: int):
        ...
        final_bytes = await num_to_binary(final_value, 1)
        return final_bytes

    return apply_dynamic_extvig_to_file(file, bytes_group=1, func=encrypt_bytes)
...
async def decrypt_file_service(key: AllStringType, file: UploadFile):
    ...
    return apply_dynamic_extvig_to_file(file, bytes_group=1, func=decrypt_bytes)
